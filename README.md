 ...
 * **Hero movement and combat** – move with `WASD` or arrow keys and aim with the mouse.  Hold the left mouse button to fire in the direction of the cursor.  The hero uses a high‑quality 8‑direction animation from our character pack; a fallback spritesheet is used if the optional assets are not found.  A dash ability (hold Shift) multiplies your speed and grants temporary invulnerability.
 * **Enemy variety** – melee enemies chase the player; ranged enemies move slower but shoot projectiles on a cooldown.  **Turret enemies** sit in place but unleash bullets in all eight directions.  **Splitter enemies** break apart into two smaller minis upon death.  **Teleporters** periodically vanish and reappear elsewhere in the room.  **Bosses** appear in the final room, firing radial spreads of bullets and periodically summoning additional minions.  All enemies now use tinted skeleton or monster animations for a cohesive fantasy look instead of simple squares.
 * **Potions** – randomly spawn and heal the player by 25 health points upon collection.  Potions and coins now use small pixel art icons instead of coloured squares.
 * **Upgrades** – after clearing a room (except the final boss room) a random upgrade item appears.  Upgrades can increase bullet damage, decrease your firing cooldown, boost movement speed or increase maximum health.  Additional upgrades introduced in later iterations include **Spread Shot** (fire multiple bullets at once), **Shield** (absorb incoming hits), **Bomb** (gain a deployable bomb that explodes after a short fuse) and more.  Each upgrade is represented by a small icon (sword, lightning bolt, boots, heart, bullets, shield or bomb) drawn procedurally at runtime.
 * **Heads‑up display, minimap and menus** – displays current health, room number and gold during gameplay.  A minimap in the top right corner shows the dungeon layout and visited rooms.  A start menu shows your best time and kill count, and the game over screen summarises your performance.  Pausing the game with **P** freezes all updates.  When your hero dies or you win, the run ends and you’re automatically returned to the main menu, so you can start a new run without quitting the game.
